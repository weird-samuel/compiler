        ...
        while self.curChar != '\0':
            ...
            if self.curChar == '+':
                ...
                if self.peek() == '*':
                    ...
                    self.nextChar()
                    token = Token("TT_POW", lastChar + self.curChar,
                                  self.curPos - 1, self.curPos)
                else:
                ...
                if self.peek() == '=':
                    lastChar = self.curChar
                    self.nextChar()
                    token = Token("TT_EQEQ", lastChar +
                                  self.curChar, self.curPos-1, self.curPos)
                else:
                ...
                if self.peek() == '=':
                    lastChar = self.curChar
                    self.nextChar()
                    token = Token("TT_NTEQ", lastChar +
                                  self.curChar, self.curPos, self.curPos)
                else:
                ...
                if self.source[startPos - 1].isdigit():
                    self.abort(
                        f"Invalid Identifier: {self.source[startPos-1 : self.curPos +1]}")

                tokText = self.source[startPos: self.curPos + 1]
                keyword = isKeyWord(tokText)
                if not keyword:
                    token = IdentToken("TT_IDENT", tokText,
                                       startPos, self.curPos)
                else:
